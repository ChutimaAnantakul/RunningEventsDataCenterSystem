...
class LoginOrganizer extends Component {
    ...
    state = {
      ...
      static contextType = AuthOrgContext;
  ...
  switchModeHandler = () => {
    this.setState((prevState) => {
      return { isLoginOrg: !prevState.isLoginOrg };
    });
  };
      ...
      submitHandler = (user) => {
        ...
        fetch("http://localhost:5000/graphql", {
          ...
          .then((resData) => {
            if (resData.data.loginOrg.token) {
              this.context.loginOrg(
                resData.data.loginOrg.organizerId,
                resData.data.loginOrg.token,
                resData.data.loginOrg.tokenExpiration
              );
            }
          })
          .catch((err) => {
